...
class AddCommentLike(LoginRequiredMixin, View):
    def post(self, request, pk, *args, **kwargs):
        ...
        if not is_like:
            post.likes.add(request.user)
            notification = Notification.objects.create(
                notification_type=1, from_user=request.user, to_user=post.author, comment=post)
        if is_like:
            post.likes.remove(request.user)
...
class UserSearch(View):
    def get(self, request, *args, **kwargs):
        ...
        profile_number = len(profile_list)
...
class Explore(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if explore_form.is_valid():
            ...
            return HttpResponseRedirect(f'/social/explore/tags?query={query}')
        return HttpResponseRedirect('/social/explore/tags')
